...
(function () {
    SlotZodiac.SlotClient = (function () {
        var instance = null;
        var CONNECTING = 0;
        var OPEN = 1;
        var CLOSING = 2;
        ...
        var Clazz = cc.Class.extend({
            ...
            checkAndReconnect: function () {
                ...
                do {
                    ...
                    this._connectTimeout = setTimeout(function () {
                        ...
                        if (scene) {
                            var l = new SlotZodiac.LoadingLayer("DISCONNECTED", "Please check your network connection");
            ...
            sendLogin: function (username, password) {
                ...
                var content = {
                    zn: "slot-daovang-zone",
                    un: "",
                    pw: "",
                    p: {
                        "1": token,
                        "os": "web",
                        "sid": SlotZodiac.SFS.sid || ""
                    }
                };
            ...
            getToken: function () {
                if (cc.GlobalSlotZodiac.AccessToken)
                    return cc.GlobalSlotZodiac.AccessToken;

                // var accessToken = cc.Global.GetSetting("accessToken");
                var accessToken = PlayerMe.token;
                if (accessToken)
                    return accessToken;
            ...
            _onJoinRoomListener: function (msgType, data) {
                SlotZodiac.SFS.roomId = data["p"]["1"];
                var scene = cc.director.getRunningScene();
                // if (scene && scene.loadingLayer) {
                //     scene.loadingLayer.removeFromParent(true);
                //     scene.loadingLayer = null;
                //     scene.stopRolling && scene.stopRolling();
                // } else {
                //     if (this._currentScene) {
                //         this._currentScene.loadingLayer && this._currentScene.loadingLayer.removeFromParent(true);
                //         this._currentScene.loadingLayer = undefined;
                //         this._currentScene = null;
                //         return;
                //     }
                ...
                scene = new SlotZodiac.SlotScene();
                // scene.lobbyNode.visible = true;
                scene.gameLayer.visible = true;
            ...
            sendExtensionRequestCurrentRoom: function (command, params) {
                if (SlotZodiac.SFS.roomId < 0)
                    return;
                this.sendExtensionRequest(SlotZodiac.SFS.roomId, command, params);
            }
        });
