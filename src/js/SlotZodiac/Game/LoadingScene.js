...
(function () {
    var res = SlotZodiac.res;
    var res_initial = SlotZodiac.res_initial;
    var res_texture = SlotZodiac.res_texture;

    SlotZodiac.LoadingScene = cc.Scene.extend({
        ctor: function () {
            this._super();
            var background = new cc.Sprite("res/SlotZodiac/loading_maya_bg.png");
            cc.GlobalSlotZodiac.scaleBackground(background);
            background.setPosition(cc.winSize.width / 2, cc.winSize.height / 2);
            this.addChild(background);

            var loadingSprite = new cc.Sprite("res/SlotZodiac/loading.png");
        ...
        loadResources: function (index) {
            ...
            if (cc.loader.getRes(res[index])) {
                if (index >= res.length - 1) {
                    ...
                    SlotZodiac.SlotClient.getInstance().connect();
                }
                else {
