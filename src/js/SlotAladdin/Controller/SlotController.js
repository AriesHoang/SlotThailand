...
(function () {
    ...
    var SlotClient = SlotAladdin.SlotClient;
    ...
    var SlotController = BaseController.extend({
        ctor: function () {
            ...
            SlotAladdin.SlotClient.getInstance().addExtensionListener("4", this._onSlotResult, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("41", this._onCreditChange, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("14", this._onJackpotUpdate, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("13", this._onUpdateBetAmount, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("39", this._onJackpotSandboxUpdate, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("2", this._onLineConfig, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("24", this._onJoinSandbox, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("35", this._onSessionInfo, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("38", this._onUpdateFreeSpin, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("61", this._onUpdatePortalFreeSpin, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("36", this._onResumeState, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("37", this._onUpdateGoldenTime, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("48", this._onUpdatePotentialDoubleAmount, this);
            // SlotAladdin.SlotClient.getInstance().addExtensionListener("7", this._onUpdateBetLevel, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("___err___", this._onError, this);
        ...
        _onLineConfig: function () {
            this._view.configLine(SlotAladdin.Lines);
        },
        ...
        sendRollRequest: function (betLevel, lines, cheatCode, skipX2) {
            ...
            if (cheatCode && cheatCode.length)
                sendObj["3"] = cheatCode;
            sendObj["0"] = skipX2;
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("6", sendObj);
        },

        sendCheatBonusRequest: function (betLevel, lines) {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("16", {1: betLevel, 2: lines});
        },

        sendJoinSlotRequest: function () {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("8", null);
        },

        sendCheatFSRequest: function (betLevel, lines) {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("20", {1: betLevel, 2: lines});
        },

        sendSandboxRequest: function (betLevel) {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("23", {1: betLevel});
        },
        ...
        sendSandBoxRollRequest: function (betLevel, lines, cheatCode, skipX2) {
            ...
            if (cheatCode && cheatCode.length)
                sendObj["3"] = cheatCode;
            sendObj["0"] = skipX2;
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("25", sendObj);
        },

        sendGetBalanceRequest: function () {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("41", null);
        },

        sendSetBetLevelRequest: function (betId) {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("36", {1: betId});
        }
    });

    SlotAladdin.SlotBonus = SlotBonus;
    SlotAladdin.SlotError = SlotError;
    SlotAladdin.SlotController = SlotController;
})();
