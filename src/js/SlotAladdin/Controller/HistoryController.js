...
(function () {
    ...
    SlotAladdin.HistoryController = BaseController.extend({
        ctor: function () {
            this._super.apply(this, arguments);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("60", this.onDataReceived, this);
            SlotAladdin.SlotClient.getInstance().addExtensionListener("22", this.onDetailReceived, this);
            //SlotAladdin.SlotClient.getInstance().addExtensionListener("10", this.onDataReceived, this);
            this.sendRequestData(0);
        },

        sendRequestData: function (page) {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("60", {1: page * 10, 2: 10});
        },

        sendRequestDetail: function (session) {
            SlotAladdin.SlotClient.getInstance().sendExtensionRequestCurrentRoom("22", {1: session});
        },
