...
(function () {
    ...
    var SlotClient = SlotHero.SlotClient;
    ...
    var SlotController = BaseController.extend({
        ctor: function () {
            ...
            SlotHero.SlotClient.getInstance().addExtensionListener("4", this._onSlotResult, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("41", this._onCreditChange, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("14", this._onJackpotUpdate, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("13", this._onUpdateBetAmount, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("39", this._onJackpotSandboxUpdate, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("2", this._onLineConfig, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("24", this._onJoinSandbox, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("35", this._onSessionInfo, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("38", this._onUpdateFreeSpin, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("61", this._onUpdatePortalFreeSpin, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("36", this._onResumeState, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("37", this._onUpdateGoldenTime, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("48", this._onUpdatePotentialDoubleAmount, this);
            // SlotHero.SlotClient.getInstance().addExtensionListener("7", this._onUpdateBetLevel, this);
            SlotHero.SlotClient.getInstance().addExtensionListener("___err___", this._onError, this);
        ...
        _onLineConfig: function () {
            this._view.configLine(SlotHero.Lines);
        },
        ...
        sendRollRequest: function (betLevel, lines, cheatCode, skipX2) {
            ...
            if (cheatCode && cheatCode.length)
                sendObj["3"] = cheatCode;
            sendObj["0"] = skipX2;
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("6", sendObj);
        },

        sendCheatBonusRequest: function (betLevel, lines) {
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("16", {1: betLevel, 2: lines});
        },

        sendJoinSlotRequest: function () {
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("8", null);
        },

        sendCheatFSRequest: function (betLevel, lines) {
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("20", {1: betLevel, 2: lines});
        },

        sendSandboxRequest: function (betLevel) {
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("23", {1: betLevel});
        },
        ...
        sendSandBoxRollRequest: function (betLevel, lines, cheatCode, skipX2) {
            ...
            if (cheatCode && cheatCode.length)
                sendObj["3"] = cheatCode;
            sendObj["0"] = skipX2;
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("25", sendObj);
        },

        sendGetBalanceRequest: function () {
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("41", null);
        },

        sendSetBetLevelRequest: function (betId) {
            SlotHero.SlotClient.getInstance().sendExtensionRequestCurrentRoom("36", {1: betId});
        }
    });

    SlotHero.SlotBonus = SlotBonus;
    SlotHero.SlotError = SlotError;
    SlotHero.SlotController = SlotController;
})();
