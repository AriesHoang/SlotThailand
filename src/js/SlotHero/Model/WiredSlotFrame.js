...
(function () {
    var SlotFrame = SlotHero.SlotFrame;
    var AllSlotObject = SlotHero.AllSlotObject;
    var WireAllLineEffect = [];
    var SymbolEffect = [];
    var QueueActions = [];
    var WireAction = [];

    SlotHero.WiredSlotFrame = SlotFrame.extend({
        ...
        _implementWires: function () {
            ...
            for (var i = 0; i < this.wires.length; i++) {
                var w = new cc.Sprite("res/SlotHero/payline/Payline_" + (i + 1) + ".png");
                // if (i === 0)
                //     w.setPosition(-20, 50);
                // else if (i === 1)
        ...
        _revealSymbol: function (symbolValue) {
            ...
            for (var i = 0; i < this.column; i++) {
                for (var j = 0; j < this.row; j++) {
                    ...
                    var eff = sp.SkeletonAnimation.createWithJsonFile("res/SlotHero/spine/effect_sym_fire.json", "res/SlotHero/spine/effect_sym_fire.atlas");
                    eff.setAnimation(0, "animation", true);
                    eff.setPosition(slotObj.x, slotObj.y - 150);
                    eff.retain();
                    // var eff = new cc.Sprite("#Sym_Effect.png");
                    // eff.setPosition(slotObj.x, slotObj.y);
        ...
        _revealReward: function (wire, value, wireIndex) {
            ...
            if (listSymHasEff.length > 0) {
                for (var i = 0; i < listSymHasEff.length; i++) {
                    ...
                    eff.retain();
                    // var eff = new cc.Sprite("#Sym_Effect.png");
                    // eff.setPosition(listSymHasEff[i].x, listSymHasEff[i].y);
                    SymbolEffect.push(eff);
                    this.containerLayout.addChild(eff, 10);
                    var thiz = this;
                    var delayAct = new cc.DelayTime(1.5);
