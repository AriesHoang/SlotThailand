...
(function () {
    ...
    var SlotClient = SlotWomenAgent.SlotClient;
    ...
    var SlotController = BaseController.extend({
        ctor: function () {
            ...
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("4", this._onSlotResult, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("41", this._onCreditChange, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("14", this._onJackpotUpdate, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("13", this._onUpdateBetAmount, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("39", this._onJackpotSandboxUpdate, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("2", this._onLineConfig, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("24", this._onJoinSandbox, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("35", this._onSessionInfo, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("38", this._onUpdateFreeSpin, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("61", this._onUpdatePortalFreeSpin, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("36", this._onResumeState, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("37", this._onUpdateGoldenTime, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("48", this._onUpdatePotentialDoubleAmount, this);
            // SlotWomenAgent.SlotClient.getInstance().addExtensionListener("7", this._onUpdateBetLevel, this);
            SlotWomenAgent.SlotClient.getInstance().addExtensionListener("___err___", this._onError, this);
        ...
        _onLineConfig: function () {
            this._view.configLine(SlotWomenAgent.Lines);
        },
        ...
        sendRollRequest: function (betLevel, lines, cheatCode, skipX2) {
            ...
            if (cheatCode && cheatCode.length)
                sendObj["3"] = cheatCode;
            sendObj["0"] = skipX2;
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("6", sendObj);
        },

        sendCheatBonusRequest: function (betLevel, lines) {
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("16", {1: betLevel, 2: lines});
        },

        sendJoinSlotRequest: function () {
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("8", null);
        },

        sendCheatFSRequest: function (betLevel, lines) {
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("20", {1: betLevel, 2: lines});
        },

        sendSandboxRequest: function (betLevel) {
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("23", {1: betLevel});
        },
        ...
        sendSandBoxRollRequest: function (betLevel, lines, cheatCode, skipX2) {
            ...
            if (cheatCode && cheatCode.length)
                sendObj["3"] = cheatCode;
            sendObj["0"] = skipX2;
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("25", sendObj);
        },

        sendGetBalanceRequest: function () {
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("41", null);
        },

        sendSetBetLevelRequest: function (betId) {
            SlotWomenAgent.SlotClient.getInstance().sendExtensionRequestCurrentRoom("36", {1: betId});
        }
    });

    SlotWomenAgent.SlotBonus = SlotBonus;
    SlotWomenAgent.SlotError = SlotError;
    SlotWomenAgent.SlotController = SlotController;
})();
